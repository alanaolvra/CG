...
def carregar_restaurante():
    global restaurante_modelo, texturas_carregadas, restaurante_display_list
    try:
        restaurante_modelo = pywavefront.Wavefront(
            'script/restaurante.obj',
        ...
        # Gera display list
        restaurante_display_list = glGenLists(1)
        glNewList(restaurante_display_list, GL_COMPILE)
        for mesh in restaurante_modelo.mesh_list:
...
def desenhar_restaurante(ladox, ladoz):
    global restaurante_modelo, restaurante_display_list

    if restaurante_modelo is None:
        carregar_restaurante()
        bbox_restaurante = calcular_bounding_box(restaurante_modelo)
        bbox_restaurante = transformar_bounding_box(bbox_restaurante, [1.8, 1, 1.5], [-10, 0, 12])
        objetos_colisao["restaurante"] = bbox_restaurante
    ...
    if restaurante_display_list:
        glCallList(restaurante_display_list)
