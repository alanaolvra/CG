...
feira_modelo = None
feira_display_list = None
texturas_carregadas = {}

def carregar_feira():
    global feira_modelo, texturas_carregadas, feira_display_list
    try:
        feira_modelo = pywavefront.Wavefront(
            'script/feira.obj',
        ...
        # Carrega texturas
        for nome_material, material in feira_modelo.materials.items():
        ...
        # Gera display list
        feira_display_list = glGenLists(1)
        glNewList(feira_display_list, GL_COMPILE)
        for mesh in feira_modelo.mesh_list:
        ...
        print("[✓] Modelo feira carregado e cacheado com sucesso")

    except Exception as e:
        print(f"[Erro] Falha ao carregar modelo feira: {e}")
...
def desenhar_feira(ladox, ladoz):
    global feira_modelo, feira_display_list

    if feira_modelo is None:
        carregar_feira()
        bbox_feira = calcular_bounding_box(feira_modelo)
        bbox_feira = transformar_bounding_box(bbox_feira, [1.3, 1, 2.4], [-10, 0, -10])
        objetos_colisao["feira"] = bbox_feira
    ...
    if feira_display_list:
        glCallList(feira_display_list)
